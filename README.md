...
packaging_downtime_logs: Lịch sử các lần dừng máy trong quá khứ (dùng cho phân tích thống kê và tự học).
log_id, timestamp, machine_id, error_code, duration_minutes, shift, technician_name, resolution_notes.
machine_telemetry: Stream cảm biến mô phỏng dòng dữ liệu IoT/SCADA.
id, timestamp, machine_id, sensor_name, sensor_value, unit.
maintenance_tickets: Quản lý các phiếu lệnh bảo trì do Agent tự động tạo.
...
Thiết kế Dual-Data (SQLite + ChromaDB RAG).

Triển khai Resilient Embedder (Google GenAI + Local Fallback).

Xây dựng 4 Industrial Tools với Schema Enum chặt chẽ (P1-CRITICAL, P2-MEDIUM, P3-LOW).

Thiết lập Verification Guardrails (Deduplication Check, Safety LOTO Enforcement).

Lọc nhiễu False Positive trên luồng cảm biến.

Hoàn thiện vòng lặp đóng (Closed-loop feedback & learning).
